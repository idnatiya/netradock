            ...
            <table class="table table-vcenter table-stack dd-table">
              ...
              <tbody>
                <tr v-for="v in sorted" :key="v.name">
                  ...
                  <td class="text-end"><button class="btn btn-icon btn-ghost-danger row-actions" type="button" :aria-label="`Hapus ${v.name}`" title="Hapus" @click="remove(v)"><IconTrash :size="18" /></button></td>
                </tr>
              </tbody>
            </table>
          </div>
